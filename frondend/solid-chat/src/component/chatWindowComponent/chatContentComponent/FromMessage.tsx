...
const FromMessage = ({ message }: { message: MessageSchema }) => {
  ...
  return (
    ...
    // display the message with some styling

    <div
      className="col-start-3 col-end-13 px-3 rounded-lg"
      ...
      <div className="flex items-end justify-start flex-row-reverse">
        <div
          className={`flex items-center justify-center h-10 w-10 rounded-full flex-shrink-0 ${
            AVATAR_COLORS[details!.id % AVATAR_COLORS.length]
          }`}
        >
        ...
        <p className="mr-2 text-xs text-gray-400 italic">
          {showTimestamp
            ? formattedTime
